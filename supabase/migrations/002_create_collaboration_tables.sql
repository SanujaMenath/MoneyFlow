...
-- Migration 002: collaboration schema for Supabase

-- ==============================================================================
-- 0. Profiles Table Integration
-- ==============================================================================
...
CREATE INDEX IF NOT EXISTS idx_shared_invitations_token ON shared_invitations(token);
CREATE INDEX IF NOT EXISTS idx_shared_invitations_email ON shared_invitations(invited_email);
CREATE INDEX IF NOT EXISTS idx_shared_invitations_list_id ON shared_invitations(list_id);
...
CREATE INDEX IF NOT EXISTS idx_shared_transactions_list_id ON shared_transactions(list_id);
CREATE INDEX IF NOT EXISTS idx_shared_transactions_creator_id ON shared_transactions(creator_id);
...
CREATE INDEX IF NOT EXISTS idx_shared_transaction_splits_tx_id ON shared_transaction_splits(transaction_id);
CREATE INDEX IF NOT EXISTS idx_shared_transaction_splits_user_id ON shared_transaction_splits(user_id);
...
CREATE INDEX IF NOT EXISTS idx_shared_activity_logs_list_id ON shared_activity_logs(list_id);
CREATE INDEX IF NOT EXISTS idx_shared_activity_logs_created_at ON shared_activity_logs(created_at DESC);
...
-- Shared Lists
DROP POLICY IF EXISTS "shared_lists_select" ON shared_lists;
...
DROP POLICY IF EXISTS "shared_lists_insert" ON shared_lists;
...
DROP POLICY IF EXISTS "shared_lists_delete" ON shared_lists;
CREATE POLICY "shared_lists_delete" ON shared_lists
  FOR DELETE USING (owner_id = auth.uid());

-- Members
DROP POLICY IF EXISTS "shared_list_members_select" ON shared_list_members;
...
DROP POLICY IF EXISTS "shared_invitations_insert" ON shared_invitations;
...
-- Shared Transactions
DROP POLICY IF EXISTS "shared_transactions_select" ON shared_transactions;
CREATE POLICY "shared_transactions_select" ON shared_transactions
  FOR SELECT USING (
    list_id IN (SELECT list_id FROM shared_list_members WHERE user_id = auth.uid())
  );

DROP POLICY IF EXISTS "shared_transactions_insert" ON shared_transactions;
...
DROP POLICY IF EXISTS "shared_transactions_delete" ON shared_transactions;
...
-- Splits
DROP POLICY IF EXISTS "shared_transaction_splits_select" ON shared_transaction_splits;
...
DROP POLICY IF EXISTS "shared_transaction_splits_insert" ON shared_transaction_splits;
...
DROP POLICY IF EXISTS "shared_transaction_splits_delete" ON shared_transaction_splits;
...
-- Activity Logs
DROP POLICY IF EXISTS "shared_activity_logs_select" ON shared_activity_logs;
CREATE POLICY "shared_activity_logs_select" ON shared_activity_logs
  FOR SELECT USING (
    list_id IN (SELECT list_id FROM shared_list_members WHERE user_id = auth.uid())
  );

DROP POLICY IF EXISTS "shared_activity_logs_insert" ON shared_activity_logs;
